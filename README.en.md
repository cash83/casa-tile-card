...
- **The "previous period" can show the closed period's cost too** (`cost_attr`, usually `last_period`): the bill arrives when the meter has already started over.

- **New row: "Bill".** The kWh and the euros of the **billing period**, which is
  not the calendar month: if you are billed every two months, the month tells you
  nothing. It needs two `utility_meter` helpers with the **bimonthly** cycle, one
  on the kWh and one on the cost, picked in the editor. They reset when the new
  period starts — not when you pay, which Home Assistant knows nothing about.
...
- **An appliance can stay "live only".** Untick every row of the "Last cycle"
  box in the editor and the whole box disappears: the drawing, the state and the
  current power remain. Before, the empty column stayed there with its title, and
  unticking the last row brought them all back.
