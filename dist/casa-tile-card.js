...
function miaScheda(carta) {
  return String((carta || {}).type || "").indexOf("custom:casa-") === 0;
}

function misuraStandard(carta, largo, alto, base) {
...
function preparaEnergia(hass, cfg) {
  const st = (hass && hass.states) || {};
  const scheda = {
...
const ConAnteprima = (Base) => class extends Base {
...
const ConDisegni = (Base) => class extends Base {
  _datiBatteria(st) {
    const c = this._config;
...
const ConFinestra = (Base) => class extends Base {
...
const ConGrafici = (Base) => class extends Base {
  _disegnaCielo(tipo) {
...
const ConMusica = (Base) => class extends Base {
  ...
  // sta la coda e li' vanno dati i comandi. Il volume pero' e' della
  // cassa, non della sessione: se la MIA cassa e' agganciata a quel
  // gruppo, il cursore resta il suo. Se no la casella della Veranda,
  // ferma a zero, faceva vedere il volume del PC ufficio.
  _cassaDelVolume() {
    const c = this._config || {};
    const mia = (this._base || {}).entity;
    if (!mia || mia === c.entity) return c.entity;
    const stati = this._hass ? this._hass.states : {};
    const st = stati[c.entity];
...
const ConPezzi = (Base) => class extends Base {
...
class CasaTile extends ConMusica(ConPezzi(ConFinestra(ConAnteprima(ConGrafici(ConDisegni(HTMLElement)))))) {
